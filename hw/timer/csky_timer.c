...
static void thead_timer_init(Object *obj)
{
    ...
    bh = qemu_bh_new(thead_timer_tick0, s);
    s->timer[0] = ptimer_init(bh, PTIMER_POLICY_LEGACY);
    sysbus_init_irq(sbd, &s->irq[0]);

    s->freq[1] = thead_timer_freq;
    bh = qemu_bh_new(thead_timer_tick1, s);
    s->timer[1] = ptimer_init(bh, PTIMER_POLICY_LEGACY);
    sysbus_init_irq(sbd, &s->irq[1]);

    s->freq[2] = thead_timer_freq;
    bh = qemu_bh_new(thead_timer_tick2, s);
    s->timer[2] = ptimer_init(bh, PTIMER_POLICY_LEGACY);
    sysbus_init_irq(sbd, &s->irq[2]);

    s->freq[3] = thead_timer_freq;
    bh = qemu_bh_new(thead_timer_tick3, s);
    s->timer[3] = ptimer_init(bh, PTIMER_POLICY_LEGACY);
